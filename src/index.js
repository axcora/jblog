let config = {};
    ...
    function handleNavigation(hash) {
        ...
        } else if (hash.startsWith('tag-list/tag/')) {
            const tag = hash.split('/')[3];
    ...
    function showPage(pageId, param = null) {
        ...
        if (pageId === 'home') {
            ...
            displayTags(param);
            updateCanonicalLink(param ? `/tag-list/tag/${param}` : '/tag-list');
    ...
    function displayTags(selectedTag = null) {
        ...
        if (selectedTag) {
            ...
            allTags.forEach(tag => {
                const tagElement = document.createElement('sppan');
